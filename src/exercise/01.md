...
We can use javascript dynamic import in order to load code only when we need it. This improves load time.
...
React also has built-in support for loading modules as React components.

The module must have a `<React.Suspense>` component to render a fallback value while the user waits for the module to be loaded.
...
```
A great tool for determining if you need code splitting is the Coverage feature of the dev tools.
