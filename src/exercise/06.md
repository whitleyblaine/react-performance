...
Every perf problem is solved by less code.
"Perf death by a thousand cuts" is when you have so many components updating every time that there's a state change that it becomes a performance bottleneck, even though none of the components are slow in isolation. 
We don't want to just immediately jump to putting memoization everywhere because that can spiral out of control fast (lots of dependencies to manage and may not even solve our problem).
Instead, we want to colocate state, putting it closest to where it is needed and taking it out of the global store.
This is basically the opposite of 'lifting state'.
